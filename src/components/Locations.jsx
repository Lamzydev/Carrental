          ...
          {regions.map(region => (
            <div key={region} style={{ marginBottom: 36 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                <span style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "0.68rem", color: "#D6AB58", letterSpacing: "2.5px", textTransform: "uppercase", whiteSpace: "nowrap" }}>
                  {region}
                </span>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px,1fr))", gap: "0.75rem" }}>
                {LOCATIONS.filter(l => l.region === region).map(loc => {
                  ...
                  const Icon = loc.icon; // ← capital I — THIS is the fix

                  return (
                    <button
                      key={loc.id}
                      onClick={() => onSelect(loc.id)}
                      style={{ width: "100%", textAlign: "left", background: "#111", border: "1px solid rgba(214,171,88,0.1)", borderRadius: 12, padding: "0.9rem 1rem", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", transition: "all 0.2s" }}
                    ...
                    >
                      {/* Icon box — was {loc.emoji}, now renders the react-icon */}
                      <div style={{ width: 38, height: 38, flexShrink: 0, background: "rgba(214,171,88,0.08)", border: "1px solid rgba(214,171,88,0.15)", borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        {Icon
                          ? <Icon size={18} color="#D6AB58" />
                          : <span style={{ fontSize: "1rem" }}>📍</span>
                        }
                      </div>

                      <div>
                        <p style={{ fontFamily: "'DM Sans',sans-serif", fontWeight: 600, fontSize: "0.85rem", color: "#F0EBDC" }}>
                          {loc.name}
                        </p>
